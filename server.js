...
app.get('/health', async (req, res) => {
    ...
    if (action === 'getCategories' && url && username && password) {
        try {
            const baseUrl = `${url}/player_api.php?username=${username}&password=${password}`;
            
            // On ajoute l'appel pour les séries (get_series_categories)
            const [vodRes, liveRes, seriesRes] = await Promise.all([
                fetch(`${baseUrl}&action=get_vod_categories`).then(r => r.json()).catch(() => []),
                fetch(`${baseUrl}&action=get_live_categories`).then(r => r.json()).catch(() => []),
                fetch(`${baseUrl}&action=get_series_categories`).then(r => r.json()).catch(() => [])
            ]);

            const categories = {
                movies: Array.isArray(vodRes) ? vodRes.map(c => c.category_name) : [],
                lives: Array.isArray(liveRes) ? liveRes.map(c => c.category_name) : [],
                series: Array.isArray(seriesRes) ? seriesRes.map(c => c.category_name) : []
            };
            ...
            res.setHeader('Access-Control-Allow-Origin', '*');
            return res.status(500).json({ error: "Erreur IPTV lors de la récupération des catégories" });
        }
    }

    // Sinon, on répond le "OK" habituel
...
// --- LA ROUTE EXISTANTE BIEN FERMÉE ---
...
}); // <--- Verifie bien qu'il y a bien }); ici pour fermer !
