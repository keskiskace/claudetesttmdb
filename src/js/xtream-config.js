...
(function () {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const tvCats = [...document.querySelectorAll('.home-category-select.tv')]
            .map(select => select.value)
            .filter(val => val !== "");

        const seriesCats = [...document.querySelectorAll('.home-category-select.series')]
            .map(select => select.value)
            .filter(val => val !== "");
        // -----------------------------------------------
        ...
        try {
            ...
            // --- AJOUT : Fetch SÉRIES ---
            setProgress(35, 'Fetching Series Streams');
            let seriesJsonText;
            try {
                seriesJsonText = await robustFetch(`${base}&action=get_series`, 'series', true);
            } catch (sErr) {
                seriesJsonText = await robustFetch(`${base}&action=get_series`, 'series', false);
            }
            let seriesList = [];
            try { seriesList = JSON.parse(seriesJsonText); } catch { }
            ...
            const blacklisted = [...document.querySelectorAll('.blacklist-item:checked')].map(cb => cb.value);
            ...
            const config = {
                ...
                home_tvs_list: tvCats.length > 0 ? tvCats : undefined,
                home_series_list: seriesCats.length > 0 ? seriesCats : undefined, // Ajouté ici
                blacklisted_cats: blacklisted.length > 0 ? blacklisted : undefined,
                addonName: document.getElementById('addonName') ? document.getElementById('addonName').value.trim() : undefined
            };
